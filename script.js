...
function addLocationMarkers() {
    ...
    kabulData.locations.forEach(location => {
        ...
        const marker = L.marker(location.coordinates, { icon })
            .addTo(map)
            .bindPopup(`<b>${location.name}</b><br><button class="select-location-btn" data-name="${location.name}" data-coords="${location.coords}">انتخاب این مکان</button>`)
            .on('click', () => {
...
window.onload = function() {
    ...
    document.getElementById('addDiscountBtn')?.addEventListener('click', () => {
        ...
        // تنظیم تاریخ پیش‌فرض (یک ماه بعد)
        const futureDate = new Date();
        futureDate.setMonth(futureDate.getMonth() + 1);
        document.getElementById('discountExpiry').value = futureDate.toISOString().split('T')[0];
    });
    ...
    document.getElementById('addDiscountForm')?.addEventListener('submit', (e) => {
        ...
        const existingDiscount = Discount.getAll().find(d => d.code === code);
        if (existingDiscount) {
    ...
    document.getElementById('saveSettingsBtn')?.addEventListener('click', () => {
        const baseFareEconomy = document.getElementById('baseFareEconomy').value;
        const baseFareComfort = document.getElementById('baseFareComfort').value;
        const baseFareBike = document.getElementById('baseFareBike').value;
        const distanceRate = document.getElementById('distanceRate').value;
        
        const settings = {
            baseFareEconomy: baseFareEconomy || 50,
            baseFareComfort: baseFareComfort || 80,
            baseFareBike: baseFareBike || 30,
            distanceRate: distanceRate || 10,
            lastUpdated: new Date().toISOString()
        };
        
        localStorage.setItem('snapp_settings', JSON.stringify(settings));
        showNotification('تنظیمات سیستم ذخیره شد', 'success');
    });

    // بارگذاری تنظیمات سیستم
    const savedSettings = localStorage.getItem('snapp_settings');
    if (savedSettings) {
        const settings = JSON.parse(savedSettings);
        document.getElementById('baseFareEconomy').value = settings.baseFareEconomy;
        document.getElementById('baseFareComfort').value = settings.baseFareComfort;
        document.getElementById('baseFareBike').value = settings.baseFareBike;
...
آمار مالی:
- مجموع درآمد: ${report.totalRevenue.toLocaleString()} افغانی
- میانگین مسافت هر سفر: ${report.averageTripDistance} کیلومتر
- میانگین هزینه هر سفر: ${report.averageTripPrice} افغانی
        `;
        ...
        reportModal.innerHTML = `
            <div class="modal-content" style="width: 500px; max-height: 80vh; overflow-y: auto;">
                ...
                <div class="modal-body">
                    <pre style="white-space: pre-wrap; font-family: system-ui; direction: rtl; text-align: right;">${reportText}</pre>
                </div>
                <div class="modal-footer" style="margin-top: 20px; padding-top: 15px; border-top: 1px solid var(--border);">
                    <button class="btn-secondary" style="margin-left: 10px;" onclick="this.closest('.modal').remove()">بستن</button>
                    <button class="btn-primary" onclick="downloadReport('${encodeURIComponent(reportText)}')">دانلود گزارش</button>
                </div>
            </div>
        `;
    ...
    // دکمه پاکسازی داده‌های تست
    document.getElementById('clearTestDataBtn')?.addEventListener('click', () => {
        if (confirm('آیا از پاکسازی تمام داده‌های تست مطمئن هستید؟ این عمل قابل بازگشت نیست!')) {
            storage.clear();
            currentUser = null;
            isAdmin = false;
            localStorage.removeItem('snapp_current_user');
            localStorage.removeItem('snapp_settings');
            ...
            showNotification('داده‌های تست پاکسازی شدند', 'success');
        }
    });
...
document.addEventListener('DOMContentLoaded', function() {
    // هنگامی که صفحه تغییر می‌کند، انتخاب‌های نقشه را پاک کن
