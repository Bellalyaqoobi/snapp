...
function updateUIAfterLogin() {
    ...
    if (userAvatar && currentUser) {
        ...
        if (currentUser.profile_image) {
            userAvatar.innerHTML = `<img src="${currentUser.profile_image}" alt="${currentUser.name}">`;
        }
    }
...
function updateProfilePage() {
    ...
    // نمایش مدارک و تصاویر
    const documentsSection = document.getElementById('documentsSection');
    if (documentsSection) {
        documentsSection.innerHTML = `
            <h3 style="margin-bottom: 15px;">مدارک و تصاویر</h3>
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; margin-bottom: 20px;">
                ...
                <div class="document-card">
                    ...
                    <img src="${currentUser.profile_image}" alt="عکس پروفایل" style="width: 100%; height: 150px; object-fit: cover; border-radius: 8px; margin: 10px 0;">
                    <button class="btn-secondary" onclick="uploadProfileImage()">تغییر عکس</button>
                </div>
                ` : ''}
                
                ${currentUser.tazkira_image ? `
                <div class="document-card">
                    <div class="document-header">
                        <i class="fas fa-id-card"></i>
                        <span>تذکره</span>
                    </div>
                    <img src="${currentUser.tazkira_image}" alt="تذکره" style="width: 100%; height: 150px; object-fit: cover; border-radius: 8px; margin: 10px 0;">
                    <div style="font-size: 12px; color: var(--gray);">شماره: ${currentUser.tazkira_number}</div>
                </div>
                ` : ''}
                
                ${currentUser.role === 'driver' && currentUser.license_image ? `
                <div class="document-card">
                    <div class="document-header">
                        <i class="fas fa-id-card-alt"></i>
                        <span>لایسنس رانندگی</span>
                    </div>
                    <img src="${currentUser.license_image}" alt="لایسنس" style="width: 100%; height: 150px; object-fit: cover; border-radius: 8px; margin: 10px 0;">
                    <div style="font-size: 12px; color: var(--gray);">شماره: ${currentUser.driver_license}</div>
                </div>
                ` : ''}
            </div>
        `;
...
function loadAdminStats() {
    ...
    if (recentTripsTable) {
        ...
        recentTrips.forEach(trip => {
            const row = document.createElement('tr');
            const date = formatDate(trip.created_at);
            const statusClass = `status-${trip.status}`;
            const statusText = {
                'requested': 'درخواست شده',
                'confirmed': 'تأیید شده',
...
function loadPendingUsers() {
    ...
    users.forEach(user => {
        ...
        row.innerHTML = `
            ...
            <td>${user.phone}</td>
            <td>${roleText}</td>
            <td>${user.tazkira_number || '---'}</td>
            <td>${date}</td>
            <td>
                ${user.profile_image ? 
                  `<button class="action-btn btn-info view-image-btn" data-image="${user.profile_image}" data-title="عکس پروفایل">عکس</button>` : 
                  '---'}
                ${user.tazkira_image ? 
                  `<button class="action-btn btn-info view-image-btn" data-image="${user.tazkira_image}" data-title="تذکره">تذکره</button>` : 
                  '---'}
                ${user.role === 'driver' && user.license_image ? 
                  `<button class="action-btn btn-info view-image-btn" data-image="${user.license_image}" data-title="لایسنس">لایسنس</button>` : 
                  ''}
            </td>
            <td class="action-buttons">
    ...
    modal.innerHTML = `
        <div class="modal-content" style="width: 500px;">
            ...
            <div class="modal-body" style="text-align: center;">
                <img src="${imageSrc}" alt="${title}" style="max-width: 100%; max-height: 400px; border-radius: 8px;">
            </div>
        </div>
    `;
...
function updateUserStatus(userId, status) {
    ...
    if (user) {
        ...
        const notification = new Notification({
            ...
            message: `حساب کاربری شما ${status === 'approved' ? 'تایید' : 'رد'} شد.`,
        ...
        row.innerHTML = `
            ...
            <td>
                <div style="display: flex; gap: 5px;">
                    ${user.verified_email ? '<span class="verified-badge" title="ایمیل تایید شده"><i class="fas fa-check-circle"></i></span>' : ''}
                    ${user.verified_whatsapp ? '<span class="verified-badge" title="واتساپ تایید شده"><i class="fab fa-whatsapp"></i></span>' : ''}
                </div>
            </td>
            <td class="action-buttons">
                ${user.status === 'approved' ? 
...
function editUser(userId) {
    ...
    modal.innerHTML = `
        <div class="modal-content" style="width: 500px;">
            ...
            <div class="modal-body">
                <form id="editUserForm">
                    <div class="form-group">
                        ...
                        <input type="text" id="editUserName" value="${user.name}" class="form-input" required>
                    </div>
                    <div class="form-group">
                        <label for="editUserEmail">ایمیل:</label>
                        <input type="email" id="editUserEmail" value="${user.email}" class="form-input" required>
                    </div>
                    <div class="form-group">
                        <label for="editUserPhone">شماره تماس:</label>
                        <input type="tel" id="editUserPhone" value="${user.phone}" class="form-input" required>
                    </div>
                    <div class="form-group">
                        <label for="editUserTazkira">شماره تذکره:</label>
                        <input type="text" id="editUserTazkira" value="${user.tazkira_number}" class="form-input">
                    </div>
                    <div class="form-group">
                        <label for="editUserWallet">موجودی کیف پول:</label>
                        <input type="number" id="editUserWallet" value="${user.wallet_balance}" class="form-input" required>
                    </div>
                    ${user.role === 'driver' ? `
                    <div class="form-group">
                        <label for="editDriverStatus">وضعیت رانندگی:</label>
                        <select id="editDriverStatus" class="form-input">
                            <option value="active" ${user.driver_status === 'active' ? 'selected' : ''}>فعال</option>
                            <option value="inactive" ${user.driver_status === 'inactive' ? 'selected' : ''}>غیرفعال</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="editOnlineStatus">وضعیت آنلاین:</label>
                        <select id="editOnlineStatus" class="form-input">
                            <option value="online" ${user.online_status === 'online' ? 'selected' : ''}>آنلاین</option>
                            <option value="offline" ${user.online_status === 'offline' ? 'selected' : ''}>آفلاین</option>
                        </select>
                    </div>
                    ` : ''}
            ...
            <div class="modal-footer" style="margin-top: 20px; padding-top: 15px; border-top: 1px solid var(--border);">
                ...
                <button class="btn-primary" id="saveEditUser">ذخیره تغییرات</button>
            </div>
        </div>
    `;
    ...
    document.getElementById('saveEditUser').addEventListener('click', () => {
        ...
        const tazkira = document.getElementById('editUserTazkira').value;
        const wallet = parseInt(document.getElementById('editUserWallet').value);
        
        user.name = name;
        user.email = email;
        user.phone = phone;
        user.tazkira_number = tazkira;
        user.wallet_balance = wallet;
        
        if (user.role === 'driver') {
            user.driver_status = document.getElementById('editDriverStatus').value;
            user.online_status = document.getElementById('editOnlineStatus').value;
        }
...
function loadDrivers() {
    ...
    if (drivers.length === 0) {
        driversTable.innerHTML = `
            <tr>
                <td colspan="11" style="text-align: center; padding: 20px; color: var(--gray);">
...
function viewDriverDetails(driverId) {
    ...
    const trips = Trip.findByDriverId(driverId);
    const completedTrips = trips.filter(t => t.status === 'completed').length;
    const totalEarning = trips.filter(t => t.status === 'completed').reduce((sum, t) => sum + t.price, 0);
    ...
    modal.innerHTML = `
        <div class="modal-content" style="width: 600px;">
            <div class="modal-header">
                <h3>جزئیات راننده ${driver.name}</h3>
                <span class="close-modal">&times;</span>
            </div>
            <div class="modal-body">
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 20px;">
                    ...
                    <div class="detail-item">
                        <label>لایسنس رانندگی:</label>
                        <span>${driver.driver_license || '---'}</span>
                    </div>
                    <div class="detail-item">
                        <label>شماره تذکره:</label>
                        <span>${driver.tazkira_number || '---'}</span>
                    </div>
                    <div class="detail-item">
                        <label>تعداد سفرها:</label>
                        <span>${driver.total_trips || 0}</span>
                    </div>
                    <div class="detail-item">
                        <label>سفرهای تکمیل شده:</label>
                    ...
                    <div class="detail-item">
                        <label>وضعیت رانندگی:</label>
                        <span class="status-badge ${driver.driver_status === 'active' ? 'status-active' : 'status-inactive'}">
                            ${driver.driver_status === 'active' ? 'فعال' : 'غیرفعال'}
                        </span>
                    </div>
                    <div class="detail-item">
                        <label>وضعیت آنلاین:</label>
                ...
                <div style="margin-top: 20px;">
                    ...
                    <div style="max-height: 200px; overflow-y: auto; margin-top: 10px;">
                        ${trips.slice(0, 5).map(trip => `
                            <div style="background: var(--bg-light); padding: 10px; border-radius: 8px; margin-bottom: 5px;">
                                ...
                                <div style="display: flex; justify-content: space-between; font-size: 12px; color: var(--gray);">
                                    ...
                                    <span class="status-badge status-${trip.status}">
                                        ${trip.status === 'completed' ? 'تکمیل شده' : 
                                          ...
                                          trip.status === 'cancelled' ? 'لغو شده' : 'درخواست شده'}
                                    </span>
                                </div>
                            </div>
                        `).join('')}
                    </div>
                    ` : '<p style="text-align: center; color: var(--gray);">هیچ سفری یافت نشد</p>'}
                </div>
            </div>
        </div>
    `;
                ...
                if (trip.payment_method === 'wallet') {
                    ...
                    if (user) {
                        ...
                        const transaction = new WalletTransaction({
                            ...
                            amount: trip.price,
                            type: 'refund',
                            description: `بازگشت هزینه سفر لغو شده توسط ادمین #${trip.id}`,
                            status: 'completed'
                        });
                        transaction.save();
                    }
...
function viewAdminTripDetails(tripId) {
    ...
    let completedAt = '---';
    if (trip.started_at) {
        startedAt = formatDateTime(trip.started_at);
    }
    if (trip.completed_at) {
        completedAt = formatDateTime(trip.completed_at);
    }
...
function loadAdminDiscounts() {
    ...
    discounts.forEach(discount => {
        ...
        row.innerHTML = `
            ...
            <td>${discount.min_order > 0 ? formatCurrency(discount.min_order) : 'بدون حداقل'}</td>
            <td><span class="status-badge ${statusClass}">${statusText}</span></td>
            <td class="action-buttons">
...
function editDiscount(discountId) {
            ...
            <div class="modal-body">
                <form id="editDiscountForm">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="editDiscountDescription" class="form-input">${discount.description || ''}</textarea>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label for="editMinOrder">حداقل سفارش:</label>
                        <input type="number" id="editMinOrder" value="${discount.min_order}" min="0" class="form-input">
                    </div>
    ...
    document.getElementById('saveEditDiscount').addEventListener('click', () => {
        ...
        const description = document.getElementById('editDiscountDescription').value;
...
function replyToTicket(ticketId) {
    ...
    modal.innerHTML = `
        <div class="modal-content" style="width: 500px;">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    <label for="ticketReply">پاسخ شما:</label>
                    <textarea id="ticketReply" rows="5" class="form-input" placeholder="پاسخ خود را وارد کنید..."></textarea>
                </div>
            </div>
            <div class="modal-footer" style="margin-top: 20px; padding-top: 15px; border-top: 1px solid var(--border);">
                <button class="btn-secondary" style="margin-left: 10px;" onclick="this.closest('.modal').remove()">لغو</button>
                <button class="btn-primary" id="sendTicketReply">ارسال پاسخ</button>
            </div>
        </div>
    `;
    ...
    document.getElementById('sendTicketReply').addEventListener('click', () => {
        ...
        if (reply.trim()) {
            ...
            const notification = new Notification({
                ...
                title: 'پاسخ به درخواست پشتیبانی',
                message: `پاسخی برای درخواست پشتیبانی شما ارسال شد.`,
                type: 'info'
            });
            notification.save();
...
function viewTicket(ticketId) {
    ...
    const repliedDate = ticket.replied_at ? formatDateTime(ticket.replied_at) : '---';
                    ...
                    <div class="detail-item">
                        ...
                        <span class="priority-badge priority-${ticket.priority}">
                            ${ticket.priority === 'low' ? 'کم' : 
                              ticket.priority === 'medium' ? 'متوسط' : 'بالا'}
                        </span>
                    </div>
                    <div class="detail-item">
                        <label>تاریخ ایجاد:</label>
...
function showNotifications() {
    ...
    modal.innerHTML = `
        <div class="modal-content" style="width: 400px;">
                    ...
                    ${notifications.map(notification => `
                        <div class="notification-item ${notification.read ? 'read' : 'unread'}" data-id="${notification.id}">
                            ...
                            <div class="notification-actions">
                                <button class="btn-secondary btn-sm" onclick="window.location.href='${notification.link}'">مشاهده</button>
                            </div>
                            ` : ''}
                        </div>
                    `).join('')}
            ...
            ${notifications.length > 0 ? `
            <div class="modal-footer" style="margin-top: 20px; padding-top: 15px; border-top: 1px solid var(--border);">
                <button class="btn-secondary" id="markAllReadBtn">خواندن همه</button>
            </div>
            ` : ''}
        </div>
    `;
    ...
    // علامت‌گذاری به عنوان خوانده شده با کلیک
...
function openPaymentModal() {
    ...
    if (selectedPaymentMethod === 'wallet') {
        ...
        const walletBalance = document.getElementById('walletBalance');
        if (walletBalance && currentUser) walletBalance.textContent = formatCurrency(currentUser.wallet_balance);
        ...
        if (currentUser && currentUser.wallet_balance < currentPrice) {
            ...
            if (payWithWalletBtn) {
                ...
                payWithWalletBtn.innerHTML = '<i class="fas fa-wallet"></i> پرداخت با کیف پول';
            }
        }
    } else {
...
window.onload = function() {
    ...
    document.getElementById('mobileNotificationBtn')?.addEventListener('click', showNotifications);
    ...
    document.getElementById('registerForm')?.addEventListener('submit', (e) => {
        ...
        const licenseImageInput = userType === 'driver' ? document.getElementById('licenseImage') : null;
        ...
        if (userType === 'driver' && (!licenseImageInput || !licenseImageInput.files[0])) {
            showError('licenseImage', 'عکس لایسنس رانندگی الزامی است');
            return;
        }
        ...
        Promise.all([
        ...
        ]).then(([profileImage, tazkiraImage, licenseImage]) => {
            // اطلاعات راننده
            let driverInfo = {};
            if (userType === 'driver') {
                ...
                if (!vehicleType || !carModel || !carColor || !plateNumber || !driverLicense) {
                    showNotification('لطفاً تمام اطلاعات رانندگی را وارد کنید', 'error');
                    return;
                }
            ...
            showNotification('خطا در آپلود تصاویر', 'error');
            console.error(error);
        });
    });

    // تغییر بین تب‌های فرم ثبت‌نام بر اساس نوع کاربر
    document.getElementById('userType')?.addEventListener('change', function() {
        const userType = this.value;
        const driverFields = document.getElementById('driverFields');
        const passengerFields = document.getElementById('passengerFields');
        
        if (driverFields) driverFields.style.display = userType === 'driver' ? 'block' : 'none';
        if (passengerFields) passengerFields.style.display = userType === 'passenger' ? 'block' : 'none';
    });
...
// تابع برای پاک کردن نقشه هنگام تغییر صفحه
...
// اضافه کردن event listener برای تغییر صفحه
...
// تابع کمکی برای اعتبارسنجی واتساپ
function verifyWhatsApp(phoneNumber) {
    // این تابع در واقعیت باید با API واتساپ ارتباط برقرار کند
...
function verifyEmail(email) {
    // این تابع در واقعیت باید یک ایمیل تأیید ارسال کند
    // اینجا فقط شبیه‌سازی می‌کنیم
    return new Promise((resolve) => {
        setTimeout(() => {
            // شبیه‌سازی موفقیت‌آمیز بودن
...
function sendVerificationCode(method, destination) {
    // این تابع باید با سرویس‌های ارسال پیامک/ایمیل ارتباط برقرار کند
    // اینجا فقط شبیه‌سازی می‌کنیم
    const code = Math.floor(100000 + Math.random() * 900000);
    
    showNotification(`کد تأیید ${method === 'sms' ? 'پیامک' : 'ایمیل'} ارسال شد: ${code}`, 'info');
    
    // در واقعیت، کد باید در سرور ذخیره شود و با ورود کاربر مقایسه شود
    return Promise.resolve(code);
}

// تابع برای گزارش‌گیری
function generateReport() {
    ...
    const report = {
        ...
        activeDrivers: drivers.filter(d => d.driver_status === 'active').length,
        onlineDrivers: drivers.filter(d => d.online_status === 'online').length,
        totalTrips: trips.length,
        completedTrips: trips.filter(t => t.status === 'completed').length,
        cancelledTrips: trips.filter(t => t.status === 'cancelled').length,
        totalRevenue: trips.reduce((sum, t) => sum + t.price, 0),
        averageTripDistance: trips.length > 0 ? (trips.reduce((sum, t) => sum + t.distance, 0) / trips.length).toFixed(1) : 0,
        averageTripPrice: trips.length > 0 ? (trips.reduce((sum, t) => sum + t.price, 0) / trips.length).toFixed(0) : 0,
        walletBalanceTotal: users.reduce((sum, u) => sum + u.wallet_balance, 0),
        driverEarningsTotal: drivers.reduce((sum, d) => sum + (d.earning || 0), 0)
    };
    
    return report;
}

// تابع برای نمایش گزارش
function showReport() {
            ...
            <div class="modal-body">
                <div class="admin-stats-grid">
                    ...
                    <div class="stat-card">
                        ...
                        <div class="stat-value">${report.totalPassengers}</div>
                        <div class="stat-label">مسافرین</div>
                    </div>
                    <div class="stat-card">
                        <div class="stat-icon"><i class="fas fa-car"></i></div>
                        <div class="stat-value">${report.totalDrivers}</div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-top: 20px;">
                    <div class="detail-item">
                        <label>رانندگان فعال:</label>
                        <span>${report.activeDrivers}</span>
                    </div>
                    <div class="detail-item">
                        <label>رانندگان آنلاین:</label>
                        <span>${report.onlineDrivers}</span>
                    </div>
                    <div class="detail-item">
                        <label>سفرهای تکمیل شده:</label>
                        <span>${report.completedTrips}</span>
                    </div>
                    <div class="detail-item">
                        <label>سفرهای لغو شده:</label>
                        <span>${report.cancelledTrips}</span>
                    </div>
                    <div class="detail-item">
                        <label>میانگین مسافت:</label>
                        <span>${report.averageTripDistance} کیلومتر</span>
                    </div>
                    <div class="detail-item">
                        <label>میانگین هزینه:</label>
                        <span>${formatCurrency(report.averageTripPrice)}</span>
                    </div>
                    <div class="detail-item">
                        <label>درآمد کل:</label>
                    ...
                    <div class="detail-item">
                        <label>موجودی کیف‌پول‌ها:</label>
                        <span>${formatCurrency(report.walletBalanceTotal)}</span>
                    </div>
                    <div class="detail-item">
                        <label>درآمد رانندگان:</label>
                        <span>${formatCurrency(report.driverEarningsTotal)}</span>
                    </div>
                </div>
            </div>
            <div class="modal-footer" style="margin-top: 20px; padding-top: 15px; border-top: 1px solid var(--border);">
                <button class="btn-secondary" style="margin-left: 10px;" onclick="this.closest('.modal').remove()">بستن</button>
                <button class="btn-primary" onclick="downloadReport()">دانلود گزارش</button>
            </div>
        </div>
    `;
...
// تابع برای دانلود گزارش
function downloadReport() {
    const report = generateReport();
    
    const reportText = `
گزارش سیستم اسنپ افغانستان
تاریخ: ${report.date}

آمار کاربران:
- تعداد کل کاربران: ${report.totalUsers} نفر
- تعداد مسافرین: ${report.totalPassengers} نفر
- تعداد رانندگان: ${report.totalDrivers} نفر
- رانندگان فعال: ${report.activeDrivers} نفر
- رانندگان آنلاین: ${report.onlineDrivers} نفر

آمار سفرها:
- تعداد کل سفرها: ${report.totalTrips} سفر
- سفرهای تکمیل شده: ${report.completedTrips} سفر
- سفرهای لغو شده: ${report.cancelledTrips} سفر
- میانگین مسافت: ${report.averageTripDistance} کیلومتر
- میانگین هزینه: ${formatCurrency(report.averageTripPrice)}

آمار مالی:
- مجموع درآمد: ${formatCurrency(report.totalRevenue)}
- موجودی کیف‌پول‌ها: ${formatCurrency(report.walletBalanceTotal)}
- درآمد رانندگان: ${formatCurrency(report.driverEarningsTotal)}
    `;
...
// اضافه کردن دکمه گزارش در پنل ادمین
document.addEventListener('DOMContentLoaded', function() {
    if (isAdmin) {
        const adminDashboard = document.getElementById('dashboard-tab');
        if (adminDashboard) {
            const reportBtn = document.createElement('button');
            reportBtn.className = 'btn-primary';
            reportBtn.innerHTML = '<i class="fas fa-chart-bar"></i> مشاهده گزارش کامل';
            reportBtn.style.marginTop = '20px';
            reportBtn.onclick = showReport;
            
            adminDashboard.appendChild(reportBtn);
        }
    }
});
