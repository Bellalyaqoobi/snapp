...
function openAuthModal() {
    ...
    modal.querySelectorAll('.auth-tab').forEach(tab => {
        tab.addEventListener('click', function() {
            ...
            modal.querySelector(`.auth-form[data-tab="${tabId}"]`).classList.add('active');
...
function loadAdminDashboard() {
    ...
    const todayTrips = trips.filter(t => new Date(t.created_at).toDateString() === today);
    const todayRevenue = todayTrips.filter(t => t.status === 'completed').reduce((sum, t) => sum + (t.price || 0), 0);
    
    document.getElementById('totalUsers').textContent = users.length;
    document.getElementById('totalDrivers').textContent = drivers.length;
    document.getElementById('totalTrips').textContent = todayTrips.length;
...
function createTripsChart() {
    ...
    for (let i = 6; i >= 0; i--) {
        ...
        const trips = Trip.getAll().filter(t => 
            new Date(t.created_at).toDateString() === targetDate
        );
...
function deleteUser(userId) {
    if (confirm('آیا از حذف این کاربر مطمئن هستید؟')) {
        User.delete(userId);
        Trip.getAll().filter(t => t.user_id == userId).forEach(t => Trip.delete(t.id));
    ...
    container.innerHTML = `
        <div class="admin-table-wrapper">
            <div class="table-filters">
                <select id="tripFilter" class="form-input" onchange="filterTrips()">
                    <option value="all">همه سفرها</option>
                    <option value="today">امروز</option>
                    <option value="completed">تکمیل شده</option>
                    <option value="cancelled">لغو شده</option>
                </select>
                <input type="date" id="tripDateFilter" class="form-input" onchange="filterTrips()">
            </div>
            <table class="admin-table">
                <thead>
                    <tr>
                        ...
                        <div class="detail-row">
                            ...
                            <span class="detail-value">
                                <div class="rating-stars">
                                    ...
                                    ${trip.rating % 1 >= 0.5 ? '☆' : ''}
                                    <span class="rating-text">(${trip.rating})</span>
                                </div>
                            </span>
                        </div>
                        ${trip.rating_comment ? `
            ...
            <div class="modal-body">
                <div class="ticket-details">
                    <div class="ticket-info">
                        ...
                        <div class="info-row">
                            <span>ایمیل:</span>
                            <span>${User.findById(ticket.user_id)?.email || '---'}</span>
                        </div>
                        <div class="info-row">
                            <span>اولویت:</span>
...
function showRideStatus() {
    document.querySelectorAll('.page').forEach(page => page.classList.remove('active'));
    document.getElementById('ride-status-page').classList.add('active');
...
function openWalletModal() {
    ...
    modal.querySelectorAll('.amount-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            modal.querySelector('#customAmount').value = amount;
        });
    });
        ...
        <div class="profile-container">
            ...
            <div class="profile-level">
                <div class="level-header">
                    <h3>سطح ${userLevel}: ${rewardSystem.levels[userLevel].name}</h3>
                ...
                <p>تخفیف سطح فعلی: ${rewardSystem.levels[userLevel].discount}%</p>
            </div>
...
function loadDriverTripRequests(container) {
    if (!container || !currentUser || currentUser.online_status !== 'online') {
        if (container) {
...
function createTicketWithCategory(category) {
    ...
    const categoryNames = {
        ...
        driver: 'مشکلات راننده',
        payment: 'مشکلات مالی',
        safety: 'امنیت و حریم خصوصی',
        general: 'عمومی'
    };
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // تنظیم پیش‌فرض‌ها
    document.getElementById('economyRide')?.classList.add('selected');
        ...
        case 'daily':
            ...
            reportHTML += `
                ...
                <div class="report-stats">
                    ...
                    <p>درآمد کل: <strong>${formatCurrency(dailyTrips.filter(t => t.status === 'completed').reduce((sum, t) => sum + t.price, 0))}</strong></p>
                </div>
            `;
            break;
...
window.editDriver = viewUserDetails;
window.filterTrips = function() {
    // تابع فیلتر کردن سفرها
};
